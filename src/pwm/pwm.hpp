...
namespace pwm {
	...
	class PwmDevice {
		...
		public:
			...
			PwmDevice(struct pwm_dt_spec pwm_dt_spec);
			...
			*/
			...
			PwmDevice(PwmDevice&&) = delete;
			PwmDevice& operator=(const PwmDevice&) = delete;
			PwmDevice& operator=(PwmDevice&&) = delete;
		...
		private:
			...
			pwm::PwmDevice::SystemState system;

			pwm::PwmDevice::WaveformState waveform;
...
*
*	@invariant	If an error related to Zephyr's PWM API occurs then the code is prefixed with "PwmDevice"
*
*/
...
*
*	@invariant	All members of this struct are set at the end of every fallible method of PwmDevice
...
/**
*
*	@fn			pwm::PwmDevice::PwmDevice(struct pwm_dt_spec pwm_dt_spec)
*
...
*	@retval		PwmDeviceSet							If an error occurs when setting the waveform parameters
...
*	@post		If an error occurs when setting the waveform parameters with Zephyr's PWM API then its return value is saved in error.return_value and error.code is set to PwmDeviceSet
...
*
*	@retval		NotRunning						If the PWM is not running
*	@retval		PwmDeviceSet							If an error occurs when setting the waveform parameters
*	@retval		Ok								If no error occurs
*
*	@post		If the PWM is not running then error.code is set to NotRunning
*	@post		If an error occurs when setting the waveform parameters with Zephyr's PWM API then its return value is saved in error.return_value and error.code is set to PwmDeviceSet
...
*
*	@retval		NotSet							If the PWM waveform parameters have never been set since the creation of the instance
*	@retval		Ok								If no error occurs
*
*	@post		If the PWM waveform parameters have never been set since the creation of the instance then error.code is set to NotSet
