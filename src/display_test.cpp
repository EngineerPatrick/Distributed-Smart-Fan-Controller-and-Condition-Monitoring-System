...
int main(void) {
	z_cbf::ErrorCode error_code = z_cbf::ErrorCode::Ok;
	...
	static z_cbf::CharacterFramebuffer cb0{DEVICE_DT_GET(DT_ALIAS(DISPLAY0_ALIAS))};

	error_code = cb0.ram_clear();

	if (error_code != z_cbf::ErrorCode::Ok) {
	...
	if (error_code != z_cbf::ErrorCode::ParamStringLength) {
		gpio_pin_toggle_dt(&error_led);
	...
	if (error_code != z_cbf::ErrorCode::ParamFontIndex) {
		gpio_pin_toggle_dt(&error_led);
